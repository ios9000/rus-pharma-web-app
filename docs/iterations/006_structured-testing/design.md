# Итерация 006 — Управляемое структурированное тестирование: Архитектура и Дизайн

> Дата: 2026-02-11
> Статус: Проектирование
> Зависимость: requirements.md (006)

---

## 1. Обзор архитектуры

```
┌─────────────────────────────────────────────────────────┐
│                    ИНСТРУКТОР                            │
│  instructor/course-management.html                       │
│  ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌────────────┐ │
│  │ Группы   │ │ Модули   │ │ Сессии   │ │ Статусы    │ │
│  │ CRUD     │ │ открыть/ │ │ тестов   │ │ курсантов  │ │
│  │          │ │ закрыть  │ │          │ │            │ │
│  └────┬─────┘ └────┬─────┘ └────┬─────┘ └─────┬──────┘ │
│       │            │            │              │         │
│       └────────────┴────────────┴──────────────┘         │
│                         │                                │
│                    Supabase API                           │
│                         │                                │
└─────────────────────────┼────────────────────────────────┘
                          │
              ┌───────────┴───────────┐
              │      SUPABASE         │
              │  ┌─────────────────┐  │
              │  │ course_levels   │  │
              │  │ groups (ext)    │  │
              │  │ course_assign.  │  │
              │  │ test_sessions   │  │
              │  │ open_answers    │  │
              │  │ student_results │  │
              │  │ questions (ext) │  │
              │  └─────────────────┘  │
              └───────────┬───────────┘
                          │
┌─────────────────────────┼────────────────────────────────┐
│                    КУРСАНТ                                │
│  Рабочая тетрадь (index.html + JS)                       │
│  ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌────────────┐  │
│  │ Мои      │ │ Тест по  │ │ Открытые │ │ Сценарий   │  │
│  │ модули   │ │ модулю   │ │ вопросы  │ │ (параграф  │  │
│  │ (назнач.)│ │ (MC)     │ │ (новый)  │ │ 3)         │  │
│  └──────────┘ └──────────┘ └──────────────┘────────────┘  │
└──────────────────────────────────────────────────────────┘
```

---

## 2. Схема базы данных

### 2.1 Новые таблицы

```sql
-- ============================================================
-- Итерация 006: Структурированное тестирование — SQL миграция
-- ============================================================

-- 1. Уровни курсов
-- ---------------------------------------------------------------
CREATE TABLE course_levels (
  id SERIAL PRIMARY KEY,
  level_number INT NOT NULL UNIQUE CHECK (level_number BETWEEN 1 AND 3),
  name TEXT NOT NULL,
  description TEXT,
  created_at TIMESTAMPTZ DEFAULT now()
);

COMMENT ON TABLE course_levels IS 'Три уровня курсов: базовый, боец-спасатель, фармакология';

INSERT INTO course_levels (level_number, name, description) VALUES
  (1, 'Базовый курс тактической медицины', 'TCCC (MARCH + PAWS). Уровень 1.'),
  (2, 'Боец-спасатель', 'Тактический акцент. Уровень 2.'),
  (3, 'Фармакология и обработка ран', 'Углублённый курс. Уровень 3.');


-- 2. Справочник модулей курса
-- ---------------------------------------------------------------
CREATE TABLE course_modules (
  id SERIAL PRIMARY KEY,
  module_number INT NOT NULL UNIQUE,
  block_number INT NOT NULL CHECK (block_number BETWEEN 1 AND 4),
  block_name TEXT NOT NULL,
  module_name TEXT NOT NULL,
  description TEXT,
  sort_order INT NOT NULL DEFAULT 0
);

COMMENT ON TABLE course_modules IS 'Справочник модулей: 20+ модулей в 4 блоках';

INSERT INTO course_modules (module_number, block_number, block_name, module_name, description, sort_order) VALUES
  (1,  1, 'Фармакологические основы', 'Фармакологические основы', 'Базовые понятия, ADME, дозировки', 1),
  (2,  1, 'Фармакологические основы', 'Антибактериальная терапия', 'Выбор антибиотиков в поле', 2),
  (3,  1, 'Фармакологические основы', 'Антигистаминные препараты', 'От крапивницы до анафилаксии', 3),
  (4,  1, 'Фармакологические основы', 'Глазные и ушные инфекции', 'Протоколы лечения', 4),
  (5,  1, 'Фармакологические основы', 'Вирусные инфекции (ОРВИ)', 'Диагностика и терапия', 5),
  (6,  1, 'Фармакологические основы', 'НПВС', 'Управление болью и воспалением', 6),
  (7,  2, 'Специальные препараты и критические состояния', 'Глюкокортикостероиды', 'От аллергии до шока', 7),
  (8,  2, 'Специальные препараты и критические состояния', 'Адреналин', 'Препарат №1 в экстренной медицине', 8),
  (9,  2, 'Специальные препараты и критические состояния', 'Спазмолитики', 'Острый живот и колики', 9),
  (10, 2, 'Специальные препараты и критические состояния', 'Шок', 'Классификация и протоколы', 10),
  (11, 2, 'Специальные препараты и критические состояния', 'Инфузионная терапия', 'Управление объёмом', 11),
  (12, 2, 'Специальные препараты и критические состояния', 'Антидоты', 'Специфическая терапия отравлений', 12),
  (13, 2, 'Специальные препараты и критические состояния', 'Анальгезия', 'Лестница обезболивания ВОЗ', 13),
  (14, 3, 'Неотложные состояния', 'Септический шок', 'Ранняя диагностика и терапия', 14),
  (15, 3, 'Неотложные состояния', 'Клещевые инфекции', 'Профилактика и лечение', 15),
  (16, 3, 'Неотложные состояния', 'Бешенство', 'Протокол постэкспозиционной профилактики', 16),
  (17, 3, 'Неотложные состояния', 'Инфаркт и инсульт', 'FAST и первая помощь', 17),
  (18, 3, 'Неотложные состояния', 'Детоксикация', 'Методы и препараты', 18),
  (19, 4, 'Боевая травма', 'Огнестрельные раны', 'Особенности фармакотерапии', 19),
  (20, 4, 'Боевая травма', 'Ожоги', 'Расчёт инфузии, анальгезия, местное лечение', 20);


-- 3. Расширение существующей таблицы groups
-- ---------------------------------------------------------------
-- Таблица groups уже существует: (code TEXT PK, name, instructor, max_cadets, is_active, created_at)
-- Добавляем поле course_level_id

DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'groups' AND column_name = 'course_level_id'
  ) THEN
    ALTER TABLE groups ADD COLUMN course_level_id INT REFERENCES course_levels(id);
  END IF;
END $$;

COMMENT ON COLUMN groups.course_level_id IS 'FK на course_levels — уровень курса группы';


-- 4. Назначения модулей группам (какие модули открыты)
-- ---------------------------------------------------------------
CREATE TABLE course_assignments (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  group_code TEXT NOT NULL REFERENCES groups(code) ON DELETE CASCADE,
  module_id INT NOT NULL REFERENCES course_modules(id),
  status TEXT NOT NULL DEFAULT 'closed' CHECK (status IN ('closed', 'open', 'completed')),
  opened_at TIMESTAMPTZ,
  closed_at TIMESTAMPTZ,
  opened_by UUID REFERENCES auth.users(id),
  UNIQUE(group_code, module_id)
);

COMMENT ON TABLE course_assignments IS 'Какие модули открыты/закрыты для какой группы';


-- 5. Тестовые сессии
-- ---------------------------------------------------------------
CREATE TABLE test_sessions (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  group_code TEXT NOT NULL REFERENCES groups(code) ON DELETE CASCADE,
  session_type TEXT NOT NULL CHECK (session_type IN ('entrance', 'module', 'final')),
  title TEXT NOT NULL,
  module_id INT REFERENCES course_modules(id),  -- NULL для входного/финального
  status TEXT NOT NULL DEFAULT 'draft' CHECK (status IN ('draft', 'active', 'closed')),
  created_by UUID REFERENCES auth.users(id),
  created_at TIMESTAMPTZ DEFAULT now(),
  activated_at TIMESTAMPTZ,
  closed_at TIMESTAMPTZ
);

COMMENT ON TABLE test_sessions IS 'Сессии тестирования: входной/модульный/финальный';
COMMENT ON COLUMN test_sessions.module_id IS 'Для модульных тестов — конкретный модуль. NULL для входного/финального.';


-- 6. Привязка модулей к тестовой сессии (для входного/финального — несколько модулей)
-- ---------------------------------------------------------------
CREATE TABLE test_session_modules (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  session_id UUID NOT NULL REFERENCES test_sessions(id) ON DELETE CASCADE,
  module_id INT NOT NULL REFERENCES course_modules(id),
  UNIQUE(session_id, module_id)
);

COMMENT ON TABLE test_session_modules IS 'Какие модули включены в тестовую сессию';


-- 7. Задания с развёрнутым ответом
-- ---------------------------------------------------------------
CREATE TABLE open_answer_questions (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  module_id INT REFERENCES course_modules(id),
  text TEXT NOT NULL,
  answer_template JSONB DEFAULT '[]',
  reference_answer TEXT,
  difficulty INT DEFAULT 1 CHECK (difficulty BETWEEN 1 AND 3),
  question_subtype TEXT DEFAULT 'explanation' 
    CHECK (question_subtype IN ('explanation', 'calculation', 'scheme', 'listing')),
  sort_order INT DEFAULT 0,
  created_by UUID REFERENCES auth.users(id),
  created_at TIMESTAMPTZ DEFAULT now(),
  updated_at TIMESTAMPTZ DEFAULT now()
);

COMMENT ON TABLE open_answer_questions IS 'Задания с развёрнутым ответом';
COMMENT ON COLUMN open_answer_questions.answer_template IS 
  'Структура ответа: [{label: "Причина 1", type: "textarea"}, {label: "Расчёт", type: "textarea"}, ...]';
COMMENT ON COLUMN open_answer_questions.reference_answer IS 'Эталонный ответ (только для инструктора)';
COMMENT ON COLUMN open_answer_questions.question_subtype IS 
  'explanation=объяснение, calculation=расчёт, scheme=схема отмены, listing=перечисление';


-- 8. Результаты прохождения тестов (multiple choice)
-- ---------------------------------------------------------------
CREATE TABLE student_test_results (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  session_id UUID REFERENCES test_sessions(id),
  cadet_id TEXT NOT NULL,  -- формат: CMK48HI0HC44Q (совместимость с GSheets)
  group_code TEXT NOT NULL REFERENCES groups(code),
  question_id UUID NOT NULL REFERENCES questions(id),
  selected_option INT,
  is_correct BOOLEAN,
  answered_at TIMESTAMPTZ DEFAULT now()
);

COMMENT ON TABLE student_test_results IS 'Ответы курсантов на MC вопросы. cadet_id совместим с GSheets';


-- 9. Результаты развёрнутых ответов
-- ---------------------------------------------------------------
CREATE TABLE student_open_answers (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  session_id UUID REFERENCES test_sessions(id),
  cadet_id TEXT NOT NULL,  -- формат: CMK48HI0HC44Q
  group_code TEXT NOT NULL REFERENCES groups(code),
  question_id UUID NOT NULL REFERENCES open_answer_questions(id),
  answers JSONB DEFAULT '{}',
  instructor_grade TEXT CHECK (instructor_grade IN ('accepted', 'needs_work', NULL)),
  instructor_comment TEXT,
  answered_at TIMESTAMPTZ DEFAULT now(),
  graded_at TIMESTAMPTZ
);

COMMENT ON TABLE student_open_answers IS 'Ответы курсантов на развёрнутые вопросы';
COMMENT ON COLUMN student_open_answers.answers IS 'JSON с ответами: {field_0: "текст", field_1: "расчёт", ...}';


-- 10. Расширение таблицы questions
-- ---------------------------------------------------------------
-- Добавляем поле module_id если его нет
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'questions' AND column_name = 'module_id'
  ) THEN
    ALTER TABLE questions ADD COLUMN module_id INT REFERENCES course_modules(id);
  END IF;
END $$;

COMMENT ON COLUMN questions.module_id IS 'FK на course_modules — привязка вопроса к модулю';


-- 11. Триггеры updated_at
-- ---------------------------------------------------------------
CREATE TRIGGER open_answer_questions_updated_at
  BEFORE UPDATE ON open_answer_questions
  FOR EACH ROW EXECUTE FUNCTION update_updated_at();


-- 12. Индексы
-- ---------------------------------------------------------------
CREATE INDEX idx_course_assignments_group ON course_assignments(group_code);
CREATE INDEX idx_test_sessions_group ON test_sessions(group_code);
CREATE INDEX idx_student_results_session ON student_test_results(session_id);
CREATE INDEX idx_student_results_student ON student_test_results(cadet_id, group_code);
CREATE INDEX idx_student_open_answers_session ON student_open_answers(session_id);
CREATE INDEX idx_questions_module ON questions(module_id);
```

### 2.2 RLS политики

```sql
-- ============================================================
-- RLS политики для итерации 006
-- ============================================================

-- course_levels: все могут читать
ALTER TABLE course_levels ENABLE ROW LEVEL SECURITY;
CREATE POLICY "course_levels_read" ON course_levels FOR SELECT USING (true);

-- course_modules: все могут читать
ALTER TABLE course_modules ENABLE ROW LEVEL SECURITY;
CREATE POLICY "course_modules_read" ON course_modules FOR SELECT USING (true);

-- groups: таблица уже существует, RLS уже настроен.
-- Если RLS не включён — включить:
-- ALTER TABLE groups ENABLE ROW LEVEL SECURITY;
-- CREATE POLICY "groups_read" ON groups FOR SELECT USING (true);
-- CREATE POLICY "groups_manage" ON groups FOR ALL USING (
--   auth.jwt() ->> 'role' = 'instructor' 
--   OR (auth.jwt() -> 'user_metadata' ->> 'role') = 'instructor'
-- );

-- course_assignments: инструктор управляет, все читают
ALTER TABLE course_assignments ENABLE ROW LEVEL SECURITY;
CREATE POLICY "course_assignments_read" ON course_assignments FOR SELECT USING (true);
CREATE POLICY "course_assignments_manage" ON course_assignments FOR ALL USING (
  auth.jwt() ->> 'role' = 'instructor' 
  OR (auth.jwt() -> 'user_metadata' ->> 'role') = 'instructor'
);

-- test_sessions: инструктор управляет, все читают
ALTER TABLE test_sessions ENABLE ROW LEVEL SECURITY;
CREATE POLICY "test_sessions_read" ON test_sessions FOR SELECT USING (true);
CREATE POLICY "test_sessions_manage" ON test_sessions FOR ALL USING (
  auth.jwt() ->> 'role' = 'instructor' 
  OR (auth.jwt() -> 'user_metadata' ->> 'role') = 'instructor'
);

-- test_session_modules: инструктор управляет, все читают
ALTER TABLE test_session_modules ENABLE ROW LEVEL SECURITY;
CREATE POLICY "test_session_modules_read" ON test_session_modules FOR SELECT USING (true);
CREATE POLICY "test_session_modules_manage" ON test_session_modules FOR ALL USING (
  auth.jwt() ->> 'role' = 'instructor' 
  OR (auth.jwt() -> 'user_metadata' ->> 'role') = 'instructor'
);

-- open_answer_questions: инструктор управляет, все читают
ALTER TABLE open_answer_questions ENABLE ROW LEVEL SECURITY;
CREATE POLICY "open_answer_questions_read" ON open_answer_questions FOR SELECT USING (true);
CREATE POLICY "open_answer_questions_manage" ON open_answer_questions FOR ALL USING (
  auth.jwt() ->> 'role' = 'instructor' 
  OR (auth.jwt() -> 'user_metadata' ->> 'role') = 'instructor'
);

-- student_test_results: все могут вставлять и читать свои
ALTER TABLE student_test_results ENABLE ROW LEVEL SECURITY;
CREATE POLICY "student_results_insert" ON student_test_results FOR INSERT WITH CHECK (true);
CREATE POLICY "student_results_read" ON student_test_results FOR SELECT USING (true);

-- student_open_answers: все могут вставлять, инструктор может обновлять (grading)
ALTER TABLE student_open_answers ENABLE ROW LEVEL SECURITY;
CREATE POLICY "student_open_answers_insert" ON student_open_answers FOR INSERT WITH CHECK (true);
CREATE POLICY "student_open_answers_read" ON student_open_answers FOR SELECT USING (true);
CREATE POLICY "student_open_answers_grade" ON student_open_answers FOR UPDATE USING (
  auth.jwt() ->> 'role' = 'instructor' 
  OR (auth.jwt() -> 'user_metadata' ->> 'role') = 'instructor'
);
```

---

## 3. Подключение Supabase к студенческому фронтенду

### CDN (как в instructor)
```html
<!-- index.html — добавить перед </head> или перед app скриптами -->
<script src="https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2"></script>
```

Инициализация — в `js/course-data.js`:
```javascript
const supabaseUrl = 'https://otoxfxwwdbeblwpizlbi.supabase.co';
const supabaseAnonKey = '...'; // тот же anon key, что в instructor/js/supabase-config.js
const supabase = window.supabase.createClient(supabaseUrl, supabaseAnonKey);
```

### Нормализация competencyId
При объединении вопросов из GSheets и Supabase — нормализовать:
```javascript
const COMPETENCY_ALIASES = { 'BASE_PHARMA': 'PHARMACOLOGY_BASICS' };
function normalizeCompetency(id) {
  return COMPETENCY_ALIASES[id] || id;
}
```

---

## 4. API — ключевые запросы

### 3.1 Сторона курсанта

```javascript
// Получить группу по group_code (из AuthModule)
const groupCode = AuthModule.getCurrentCadet().groupCode; // 'PHARMA-0126'
const cadetId = AuthModule.getCurrentCadet().id; // 'CMK48HI0HC44Q'

// Получить открытые модули для группы
const { data: assignments } = await supabase
  .from('course_assignments')
  .select('*, course_modules(*)')
  .eq('group_code', groupCode)
  .eq('status', 'open');

// Получить вопросы multiple choice по модулю
const { data: questions } = await supabase
  .from('questions')
  .select('*')
  .eq('module_id', moduleId);

// Получить задания с развёрнутым ответом по модулю
const { data: openQuestions } = await supabase
  .from('open_answer_questions')
  .select('*')
  .eq('module_id', moduleId)
  .order('sort_order');

// Сохранить ответ на MC вопрос
await supabase.from('student_test_results').insert({
  session_id, cadet_id, group_code, question_id, 
  selected_option, is_correct
});

// Сохранить развёрнутый ответ
await supabase.from('student_open_answers').insert({
  session_id, cadet_id, group_code, question_id, answers: { field_0: '...', field_1: '...' }
});
```

### 3.2 Сторона инструктора

```javascript
// Обновить группу — добавить уровень курса
await supabase.from('groups')
  .update({ course_level_id: 3 })
  .eq('code', 'PHARMA-0126');

// Открыть модуль для группы
await supabase.from('course_assignments').upsert({
  group_code: 'PHARMA-0126', module_id, status: 'open', opened_at: new Date().toISOString()
});

// Создать тестовую сессию
const { data: session } = await supabase.from('test_sessions').insert({
  group_code, session_type: 'module', title: 'Модуль 1 — тест', 
  module_id: 1, status: 'active'
}).select().single();

// Получить статус прохождения группы
const { data: results } = await supabase
  .from('student_test_results')
  .select('cadet_id, is_correct, question_id')
  .eq('group_code', groupId)
  .eq('session_id', sessionId);
```

---

## 5. UI — описание экранов

### 4.1 Курсант: выбор модуля (замена текущего test-selector.js)

```
┌─────────────────────────────────────────────┐
│  📚 Мои модули                 Группа: ФМ-25│
├─────────────────────────────────────────────┤
│                                             │
│  БЛОК I: Фармакологические основы           │
│  ┌─────────────────────────────────────┐    │
│  │ ✅ Модуль 1: Фарм. основы  [90%]   │    │
│  │ 🔵 Модуль 2: Антибиотики   [новый] │    │
│  │ 🔒 Модуль 3: Антигистаминные       │    │
│  │ 🔒 Модуль 4: Глазные/ушные         │    │
│  └─────────────────────────────────────┘    │
│                                             │
│  БЛОК II: (закрыт)                          │
│  🔒 Модули 7–13                             │
│                                             │
└─────────────────────────────────────────────┘
```

Статусы модуля: 🔒 закрыт → 🔵 новый (назначен) → 🟡 в процессе → ✅ завершён → 🔄 повторный

### 4.2 Курсант: тест по модулю

```
┌─────────────────────────────────────────────┐
│  ← Назад    Модуль 1: Фарм. основы         │
├─────────────────────────────────────────────┤
│  📋 Параграф 2: Рабочая тетрадь            │
│                                             │
│  ── Multiple Choice (5 из 12) ──            │
│  Вопрос 5:                                  │
│  Какой путь введения обеспечивает           │
│  100% биодоступность?                       │
│                                             │
│  ○ Пероральный                              │
│  ● Внутривенный                             │
│  ○ Внутримышечный                           │
│  ○ Подкожный                                │
│                                             │
│  [← Предыдущий]  [Следующий →]              │
│                                             │
│  ── Задания с развёрнутым ответом ──        │
│  ⬜ Задание 1: Расчёт дозы преднизолона     │
│  ⬜ Задание 2: Причины запрета в/м при шоке │
│                                             │
│  ── Параграф 3: Клинический сценарий ──     │
│  ⬜ Сценарий: Анафилаксия в поле            │
└─────────────────────────────────────────────┘
```

### 4.3 Курсант: задание с развёрнутым ответом

```
┌─────────────────────────────────────────────┐
│  ← Назад    Задание 1                       │
├─────────────────────────────────────────────┤
│  Расчёт нагрузочной дозы преднизолона       │
│  при анафилактическом шоке:                 │
│                                             │
│  Исходные данные:                           │
│  • Вес пациента: 75 кг                      │
│  • Формула: 1.5–2 мг/кг                     │
│                                             │
│  Расчёт:                                    │
│  ┌─────────────────────────────────────┐    │
│  │                                     │    │
│  │                                     │    │
│  └─────────────────────────────────────┘    │
│                                             │
│  Итоговая доза:                             │
│  ┌─────────────────────────────────────┐    │
│  │                                     │    │
│  └─────────────────────────────────────┘    │
│                                             │
│  [Сохранить ответ]                          │
└─────────────────────────────────────────────┘
```

### 4.4 Инструктор: управление курсом (новая страница)

```
┌─────────────────────────────────────────────┐
│  🎓 Управление курсом                       │
├─────────────────────────────────────────────┤
│  Группа: [Фармакология-2025 ▼] [+ Новая]   │
│  Уровень: 3 — Фармакология и обработка ран  │
│                                             │
│  ── БЛОК I: Фармакологические основы ──     │
│  │ Модуль 1 │ ✅ Открыт │ 12 вопр │ [Закрыть] │
│  │ Модуль 2 │ ✅ Открыт │  8 вопр │ [Закрыть] │
│  │ Модуль 3 │ 🔒 Закрыт │ 10 вопр │ [Открыть] │
│  │ ...                                      │
│                                             │
│  ── БЛОК II: Спец.препараты ──              │
│  │ [Открыть весь блок]                      │
│  │ Модуль 7  │ 🔒 │ 6 вопр │ [Открыть]     │
│  │ ...                                      │
│                                             │
│  ── Тестовые сессии ──                      │
│  │ 📝 Входное тестирование │ Активна │ 15/20 │
│  │ 📝 Модуль 1 — тест      │ Завершена│ 20/20│
│  │ [+ Создать сессию]                       │
│                                             │
│  ── Статус курсантов ──                     │
│  │ Иванов  │ Мод.1: 90% │ Мод.2: в проц.   │
│  │ Петров  │ Мод.1: 75% │ Мод.2: не начат   │
│  │ Сидоров │ Мод.1: 60% │ [Назначить повтор]│
└─────────────────────────────────────────────┘
```

---

## 6. Ключевые файлы проекта (изменения)

### Новые файлы:
```
instructor/
  course-management.html       — страница управления курсом
  js/course-management.js      — логика управления (группы, модули, сессии)

js/
  module-selector.js           — замена test-selector.js (выбор модуля курсантом)
  module-test.js               — тест по модулю (MC + open answer)
  open-answer-ui.js            — UI для заданий с развёрнутым ответом
  course-data.js               — загрузка данных курса (модули, назначения)
```

### Модифицируемые файлы:
```
js/test-selector.js            — рефакторинг → делегирование в module-selector.js
js/competency-progress.js      — расширение: прогресс по модулям
index.html                     — интеграция нового UI модулей
instructor/index.html           — ссылка на course-management
```

---

## 7. Идентификация курсанта

### Существующая система (Google Sheets)
В Google Sheets **уже работает** PIN-авторизация:
- Курсант получает `cadet_id` (формат: `CMK48HI0HC44Q`) и `pin_code` (4 цифры)
- Курсант входит по `group_code` + `pin_code` или `cadet_id` + `pin_code`
- Прогресс хранится в формате JSON (progress_matrix) с привязкой к `cadet_id`

### Стратегия для итерации 006
**Сохраняем существующую систему авторизации.** Не меняем и не ломаем.

Новые таблицы (`student_test_results`, `student_open_answers`) используют `cadet_id` из существующей системы. Это обеспечивает:
1. Совместимость с текущим прогрессом в Google Sheets
2. Единый идентификатор курсанта во всех системах
3. Возможность связать старые и новые результаты при миграции

### Существующие данные (из Google Sheets)
```
Группа: PHARMA-0126 | Инструктор: Овсянников | max: 30
Курсанты:
  CMK48HI0HC44Q | Игорь | PIN: 4454
  CMK4A69J300L1 | Илья  | PIN: 4904
```

### Существующий формат прогресса
```json
{
  "PHARMACOLOGY_BASICS": {
    "diagnostic": 0,
    "sections": {"1": null, "2": null, "3": null, "4": null},
    "final": null
  },
  "HEMOSTASIS": { ... },
  ...
}
```
Новые результаты модульных тестов сохраняются в `student_test_results` (Supabase), но НЕ ломают существующую `progress_matrix`.

---

## 8. Маппинг вопросов по модулям

### Результаты аудита

**81 из 88 вопросов** уже имеют числовой `module` (1–20) в Google Sheets. Маппинг тривиальный.

**7 вопросов [SB]** (из Supabase) — все относятся к PHARMACOLOGY_BASICS → Модуль 1:
- 2 вопроса с пустым `module` + компетенция PHARMACOLOGY_BASICS
- 5 вопросов с `module = "ФАРМАКОЛОГИЧЕСКИЕ ОСНОВЫ"` + компетенция PHARMACOLOGY_BASICS или пусто

**Связь Модуль ↔ Компетенция — строго 1:1:**
```
Модуль 1  → BASE_PHARMA          Модуль 11 → INFUSION_THERAPY
Модуль 2  → ANTIBIOTICS           Модуль 12 → ANTIDOTES
Модуль 3  → ANTIHISTAMINES        Модуль 13 → ANALGESIA
Модуль 4  → EYE_EAR_INFECTIONS    Модуль 14 → SEPTIC_SHOCK
Модуль 5  → VIRAL_INFECTIONS      Модуль 15 → TICK_INFECTIONS
Модуль 6  → NSAID                 Модуль 16 → RABIES
Модуль 7  → GLUCOCORTICOIDS       Модуль 17 → CARDIAC_STROKE
Модуль 8  → ADRENALINE            Модуль 18 → DETOX
Модуль 9  → SPASMOLITICA          Модуль 19 → GUNSHOT_WOUNDS
Модуль 10 → HEMORRHAGIC_SHOCK     Модуль 20 → BURNS
```

**Разделы ≠ Блоки** (важно!):
```
Раздел 1 → Модули 1-5   (Блок I, кроме мод.6)
Раздел 2 → Модули 6-9   (Блок I мод.6 + Блок II мод.7-9)
Раздел 3 → Модули 10-12, 14-15  (Блок II + Блок III)
Раздел 4 → Модули 16-20 (Блок III + Блок IV)
Раздел 5 → Модуль 13    (Блок II)
```

### Миграционный SQL-скрипт

```sql
-- Маппинг числовых модулей из Google Sheets → module_id (FK на course_modules)
-- Выполняется ПОСЛЕ создания course_modules и добавления module_id в questions

-- Для вопросов с числовым модулем (81 шт) — в Supabase это будет работать
-- только после полной миграции из GSheets. На данном этапе — для 7 вопросов [SB]:
UPDATE questions 
SET module_id = (SELECT id FROM course_modules WHERE module_number = 1)
WHERE (module IS NULL OR module = '' OR module = 'ФАРМАКОЛОГИЧЕСКИЕ ОСНОВЫ')
  AND (competency_id = 'PHARMACOLOGY_BASICS' OR competency_block ILIKE '%basics%');
```

### Стратегия
- **Фаза 1:** Заполняем `module_id` для 7 вопросов в Supabase (скрипт выше)
- **Для 81 вопроса в Google Sheets:** Используем поле `Модуль` напрямую при загрузке данных на клиенте (без миграции в БД)
- **Полная миграция GSheets → Supabase:** Отдельная будущая итерация

---

## 9. Фазы реализации для Claude Code

### Фаза 0: Аудит и подготовка (0.5 дня)
**Аудит данных ВЫПОЛНЕН** (этот чат). Результаты — в разделе 7 этого документа.

**Задание для Claude Code:**
```
Прочитай .claude/CLAUDE.md, docs/roadmap.md, и docs/iterations/006_structured-testing/requirements.md + design.md.

Аудит данных уже выполнен. Ключевые результаты:
- 81 из 88 вопросов имеют числовой модуль (1-20) в Google Sheets
- 7 вопросов [SB] в Supabase → все = Модуль 1
- Маппинг Модуль ↔ Компетенция = 1:1 (см. design.md раздел 7)
- В GSheets уже есть Группы (PHARMA-0126) и Курсанты (cadet_id + PIN)

Задачи:
1. Исследуй текущий код (верификация):
   - js/competencies-config.js — структура 26 компетенций, сверь с аудитом
   - js/test-selector.js — как сейчас выбираются тесты, что нужно менять
   - js/competency-progress.js — текущий формат прогресса
   - js/app.js или index.html — как работает PIN-авторизация
   - instructor/ — текущий кабинет инструктора, какие страницы есть
   
2. Подтверди данные в Supabase:
   - SELECT module, section, competency_block, COUNT(*) 
     FROM questions GROUP BY module, section, competency_block;
   - Совпадает ли с аудитом?
   
3. Создай docs/iterations/006_structured-testing/audit-report.md:
   - Подтверждение результатов аудита
   - Список файлов, которые нужно создать/изменить
   - Какие текущие функции затронуты

Не пиши код в этой фазе — только верификация и отчёт.
```

### Фаза 1: БД + миграция (1 день)
**Задание для Claude Code:**
```
Прочитай docs/iterations/006_structured-testing/design.md (раздел 2: Схема БД).

ВАЖНО: Таблица groups уже существует (code TEXT PK). НЕ создавай student_groups.
Вместо этого — ALTER TABLE groups ADD COLUMN course_level_id.
Модуль 21 убран — только модули 1-20.

Задачи:
1. Выполни SQL миграцию из design.md:
   - Создай таблицы: course_levels, course_modules, 
     course_assignments, test_sessions, test_session_modules,
     open_answer_questions, student_test_results, student_open_answers
   - ALTER TABLE groups ADD COLUMN course_level_id (если нет)
   - Добавь поле module_id в questions (если нет)
   - Настрой RLS
   - Создай индексы

2. Маппинг вопросов (только 7 [SB] в Supabase):
   - UPDATE questions SET module_id = (SELECT id FROM course_modules WHERE module_number = 1)
     WHERE module IS NULL OR module = 'ФАРМАКОЛОГИЧЕСКИЕ ОСНОВЫ';
   - 81 вопрос в GSheets уже имеют числовой модуль — они маппятся на клиенте

3. Подключи Supabase CDN в index.html:
   - <script src="https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2"></script>
   - Создай js/supabase-client.js (URL + anon key из instructor/js/supabase-config.js)

4. ИСПРАВЬ: instructor/js/supabase-config.js — URL должен быть .co НЕ .com

5. Обнови .claude/CLAUDE.md — добавь новые таблицы и изменения
6. Создай docs/iterations/006_structured-testing/session-log-phase1.md
```

### Фаза 2: Курсант — модули и MC тесты (2 дня)
**Задание для Claude Code:**
```
Прочитай docs/iterations/006_structured-testing/design.md (разделы 3, 5.1, 5.2, 6, 7, 8, 12).

ВАЖНО: Данные берутся из ДВУХ источников:
- Google Sheets: 81 вопрос (загружаются через существующий механизм)
- Supabase: 7 вопросов + новые таблицы (course_modules, course_assignments и др.)
Курсант авторизуется через СУЩЕСТВУЮЩУЮ PIN-систему (cadet_id + pin_code).

Задачи:
1. Создай js/course-data.js:
   - Загрузка модулей из Supabase (course_modules)
   - Загрузка назначений для группы (course_assignments)
   - Объединение вопросов из GSheets (поле Модуль) и Supabase (module_id)
   - Функции: getOpenModules(groupId), getModuleQuestions(moduleNumber)

2. Создай js/module-selector.js:
   - UI выбора модуля (заменяет текущий "все 88 вопросов")
   - Показывает только открытые модули с статусами
   - Группировка по блокам
   - Учитывает cadet_id из текущей сессии

3. Модифицируй тестовый UI:
   - Тест по конкретному модулю (фильтрация по module number)
   - Сохранение результатов в student_test_results (Supabase)
   - Прогресс по модулю

4. Интегрируй в index.html (после существующей PIN-авторизации)
5. Обеспечь обратную совместимость (старый режим "все вопросы" как fallback)
6. Создай session-log-phase2.md
```

### Фаза 3: Курсант — задания с развёрнутым ответом (1–2 дня)
**Задание для Claude Code:**
```
Прочитай docs/iterations/006_structured-testing/design.md (разделы 5.3, requirements.md раздел 6.2).

Задачи:
1. Создай js/open-answer-ui.js:
   - Рендеринг задания по answer_template
   - Поля ввода для каждого элемента шаблона
   - Сохранение ответов в student_open_answers

2. Интегрируй в module-test.js:
   - После MC вопросов — секция развёрнутых вопросов
   - Навигация: MC → Open Answer → Сценарий

3. Добавь тестовые данные:
   - 3–5 заданий с развёрнутым ответом для Модуля 1
   - Используй примеры из requirements.md (раздел 4.2 хэндоффа)

4. Создай session-log-phase3.md
```

### Фаза 4: Инструктор — управление курсом (2 дня)
**Задание для Claude Code:**
```
Прочитай docs/iterations/006_structured-testing/design.md (разделы 4, 5.4, 7, 12).

ВАЖНО: В Google Sheets УЖЕ есть группы (PHARMA-0126) и курсанты (с cadet_id, PIN).
Новая страница управления курсом работает С Supabase, но учитывает
существующие данные из GSheets.

Задачи:
1. Создай instructor/course-management.html + instructor/js/course-management.js:
   - Создание группы: group_code + name + уровень курса + instructor_name
   - Импорт существующей группы из GSheets (ввод group_code → подтягиваем)
   - Список модулей по блокам с кнопками открыть/закрыть
   - Массовое открытие блока (все модули блока)
   - Создание тестовых сессий (входной/модульный/финальный)

2. Добавь панель статусов курсантов:
   - Список курсантов группы (по cadet_id, имя)
   - Прогресс по каждому модулю (% правильных ответов)
   - Кнопка "Назначить повторное прохождение"

3. UI для привязки вопросов к модулям:
   - Таблица вопросов с dropdown для выбора модуля
   - Массовое назначение (выбрать несколько → назначить модуль)

4. Добавь ссылку на course-management в instructor/index.html
5. Создай session-log-phase4.md
```

### Фаза 5: Интеграция и тестирование (1 день)
**Задание для Claude Code:**
```
Задачи:
1. End-to-end тестирование потока:
   - Инструктор создаёт группу → открывает модули → создаёт сессию
   - Курсант видит назначенные модули → проходит тест → сохраняет ответы
   - Инструктор видит результаты

2. Проверь обратную совместимость:
   - Flashcards работают
   - Карточки препаратов работают
   - Сценарии работают
   - Google Sheets sync не нарушена

3. Обнови документацию:
   - .claude/CLAUDE.md — новые таблицы и файлы
   - docs/roadmap.md — отметь 006 как завершённую
   - docs/iterations/006_structured-testing/session-log-phase5.md

4. Git commit + push + verify deployment на GitHub Pages
```

---

## 10. Оценка сроков

| Фаза | Описание | Оценка |
|------|----------|--------|
| 0 | Аудит и верификация (частично выполнен) | 0.5 дня |
| 1 | БД + миграция | 1 день |
| 2 | Курсант — модули + MC | 2 дня |
| 3 | Курсант — open answer | 1–2 дня |
| 4 | Инструктор — управление | 2 дня |
| 5 | Интеграция + тестирование | 1 день |
| **Итого** | | **7.5–8.5 дней** |

---

## 11. Риски и решения

| Риск | Решение |
|------|---------|
| Данные в GSheets и Supabase — два источника правды | На 006 работаем с обоими: GSheets для 81 вопроса (клиент), Supabase для 7 + новые таблицы. Полная миграция — отдельная итерация |
| Существующая PIN-авторизация может конфликтовать с новым UI | Сохраняем существующий flow: cadet_id + PIN. Новые экраны модулей встраиваются ПОСЛЕ авторизации |
| Существующий прогресс (progress_matrix) несовместим с новыми результатами | Новые результаты пишутся в student_test_results (Supabase), старый progress_matrix не трогаем. Объединение — при миграции |
| 88 вопросов — мало для полноценного модульного тестирования | LLM генерация (итерация 005) позволяет быстро наращивать контент |
| Google Sheets sync может сломаться при изменении схемы questions | Добавляем module_id как nullable, Apps Script игнорирует новые поля |

---

## 12. Совместимость с Google Sheets

### Что сохраняется без изменений
- Лист «Вопросы»: 88 вопросов, поле Модуль (числовое)
- Лист «Препараты»: карточки препаратов
- Лист «Сценарии»: интерактивные клинические сценарии
- Лист «Группы»: group_code, instructor
- Лист «Курсанты»: cadet_id, pin_code
- Лист «Прогресс»: progress_matrix JSON

### Что добавляется в Supabase (параллельно)
- course_modules — справочник модулей (новый)
- course_assignments — назначения модулей группам (новый)
- test_sessions — сессии тестирования (новый)
- student_test_results — новый формат результатов MC
- student_open_answers — развёрнутые ответы (новый)
- open_answer_questions — новый тип вопросов

### Принцип двойного источника
Пока не произведена полная миграция:
- **Вопросы MC** загружаются из Google Sheets (81 шт) + Supabase (7 шт)
- **Группы и курсанты** управляются через Google Sheets (существующий flow)
- **Назначения модулей и сессии** — через Supabase (новый функционал)
- **Результаты нового тестирования** — в Supabase (student_test_results)
