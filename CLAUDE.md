...
├── js/
│   ├── supabase-client.js        ← инициализация Supabase для студенческого фронтенда
│   ├── course-data.js            ← загрузка модулей, назначений, вопросов (Supabase + appData)
│   ├── module-selector.js        ← UI выбора модуля для курсантов
│   ├── open-answer-ui.js         ← UI заданий с развёрнутым ответом
│   ├── test-module.js            ← MC тест с поддержкой модулей + Supabase save
│   └── ...                       ← остальные модули (cards, cases, progress, auth, sync)
...
│       ├── app.js                 ← роутинг секций (+ lazy-init CourseManagement)
...
│       ├── course-management.js   ← управление курсом: модули, сессии, статусы, оценка OA
...
| `test-selector.js` | Выбор тестов / вопросов для тренировки (legacy, заменён module-selector.js) |
| `course-data.js` | Загрузка модулей, назначений, вопросов из Supabase + appData. Экспорт: `CourseData` |
| `module-selector.js` | UI выбора модуля: блоки, статусы, MC + OA счётчики. Экспорт: `ModuleSelector` |
| `open-answer-ui.js` | UI заданий с развёрнутым ответом: формы, сохранение, навигация. Экспорт: `OpenAnswerUI` |
| `supabase-client.js` | Инициализация `window.supabaseClient` для студенческого фронтенда |
| `auth-*.js` | Авторизация: группы, PIN-коды, офлайн-логин, облачная синхронизация |

**Контент:** 81 вопрос (GSheets) + 7 вопросов (Supabase), 6+ препаратов, сценарии.
...
Завершена: **006 — Structured Testing** (см. docs/iterations/006_structured-testing/)
Следующая: **007 — Migration** (полная миграция с Google Sheets на Supabase)
...
| 006 | Structured Testing (структурированное тестирование) | ✅ Завершено |
